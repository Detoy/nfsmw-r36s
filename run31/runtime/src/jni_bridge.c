                  ...
                  char *error, size_t error_size)
{
    ...
    typedef unsigned char *(*native_ui_state_function)(void);
    ...
    const uintptr_t controller_instance_address = app_image->load_bias +
        0x003f7c88U;
    const uintptr_t ui_state_address = app_image->load_bias + 0x0007566cU;
    ...
    native_ui_state_function ui_state = NULL;
    ...
    (void)memcpy(&ui_state, &ui_state_address, sizeof(ui_state));
    ...
    (void)printf("G6-UIMODE pointer-mode=forced-gamepad "
                 "guest-field=ui_state+0x239\n");
    ...
    for (frame = 0U; frame_limit == 0U || frame < frame_limit; ++frame) {
        ...
        /*
         * Front-end pointer-mode override.
         *
         * Run 30 disproved the right-stick theory: a scan of the
         * car-selection screen class at 0x163000-0x16b000 shows it builds
         * only the four D-pad action ids 1002/1003/1004/1005 and no others,
         * so menu actions 1054/1055 cannot reach it and the L2/R2 taps were
         * correctly logged but inert.
         *
         * Its four D-pad handlers share one guard chain, and the last link
         * is the real gate:
         *
         *     if (this->busy_0x94)                     return;
         *     if (this->targets_begin == targets_end)   return;
         *     if (this->widget_0x150->flag_0x104)       return;
         *     if (ui_state()->byte_0x239 != 0)          return;   <-- here
         *     this->highlight_index_0x18c += step;
         *
         * `ui_state()` is the argument-less singleton accessor at 0x7566c,
         * and byte 0x239 is the title's touch-versus-gamepad mode. The
         * setters at 0x3647c4-0x364d70 always write it together with the
         * navigation-panel visibility field, storing 1 for the touch panel
         * and 0 for the MOGA panel. While it is 1 the car screen silently
         * refuses every D-pad highlight move, which is exactly what runs
         * 25, 28, 29 and 30 recorded: the drawn carousel still swiped, but
         * A always re-fired the same class button.
         *
         * This machine has no touchscreen, so gamepad mode is always the
         * correct mode. Hold it at 0 whenever the emergency cursor is off,
         * and log both the observed value and every override so the next
         * trace proves whether this was the blocking guard.
         */
        if (ui_state != NULL) {
            unsigned char *ui_state_object = ui_state();

            if (ui_state_object != NULL) {
                volatile unsigned char *pointer_mode =
                    (volatile unsigned char *)(ui_state_object + 0x239U);
                int observed = (int)*pointer_mode;

                if (observed != previous_pointer_mode) {
                    (void)printf("G6-UIMODE frame=%u value=%d\n",
                                 frame, observed);
                    previous_pointer_mode = observed;
                }
                if (cursor_visible == 0 && observed != 0) {
                    *pointer_mode = 0U;
                    (void)printf("G6-UIMODE forced frame=%u was=%d now=0\n",
                                 frame, observed);
                    previous_pointer_mode = 0;
                }
            }
        }
