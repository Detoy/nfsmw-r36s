                  ...
                  char *error, size_t error_size)
{
    ...
    (void)printf("G6-MOGA presence=forced connected=0x133 available=0x135\n");
    ...
    for (frame = 0U; frame_limit == 0U || frame < frame_limit; ++frame) {
         ...
         * MOGA presence flags.
         *
         * Run 31 ruled out the pointer-mode guard: `ui_state()+0x239` read 0
         * at frame 0 and never changed, so the car screen was never in touch
         * mode. Run 31's trace then made the real pattern unambiguous. Every
         * settled A on `car_select_new` in runs 29, 30 and 31 - twelve of
         * twelve - produced a `CLASS_FILTER_*` output, including presses made
         * straight after a D-pad Right that had visibly swiped the carousel.
         * The drawn selection moved; the highlight never did.
         *
         * `showMogaHighlight` for this screen is at 0x167a14, and it decides
         * between `mogaHighlightClass` and `mogaHighlightCar` from the
         * carousel index. Its very first guard is:
         *
         *     if ((controller->byte_0x133 | controller->byte_0x134) == 0)
         *             return;
         *
         * so with no MOGA marked present the highlight is never repositioned
         * and A keeps activating the class dropdown the layout focused first.
         * `setConnected` at 0x7913c is what should set 0x133, and it opens
         * with `if (controller->byte_0x135 == 0) return;`. Byte 0x135 is
         * written once during startup at 0x547208 from a JNI-dependent probe,
         * which this loader's bounded JNIEnv cannot answer truthfully, so the
         * state events this mapper already sends are discarded.
         *
         * Write the presence bytes directly, exactly as the proven analog
         * steering path already writes 0x138/0x13c on the same object, and
         * log all four state bytes on change so the trace shows what the
         * guest actually held.
         */
        {
            volatile unsigned char *moga_available =
                (volatile unsigned char *)controller_state + 0x135U;
            volatile unsigned char *moga_connected =
                (volatile unsigned char *)controller_state + 0x133U;
            int state_signature;

            if (*moga_available == 0U) *moga_available = 1U;
            if (*moga_connected == 0U) *moga_connected = 1U;
            state_signature =
                (int)(*((volatile unsigned char *)controller_state + 0x132U)) |
                ((int)*moga_connected << 8) |
                ((int)*((volatile unsigned char *)controller_state + 0x134U)
                 << 16) |
                ((int)*moga_available << 24);
            if (state_signature != previous_moga_state) {
                (void)printf("G6-MOGA state frame=%u mode=%d connected=%d "
                             "alt=%d available=%d\n", frame,
                             state_signature & 0xff,
                             (state_signature >> 8) & 0xff,
                             (state_signature >> 16) & 0xff,
                             (state_signature >> 24) & 0xff);
                previous_moga_state = state_signature;
            }
        }
