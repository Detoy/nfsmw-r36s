                  ...
                  char *error, size_t error_size)
{
    ...
    int previous_focus_left = 0;
    int previous_focus_right = 0;
    int cursor_x = 320;
    int cursor_y = 240;
    ...
    (void)printf("G6-MOGA focus-cycle=L2/R2 channel=AXIS_Z "
                 "actions=1054/1055 race-effect=none\n");
    ...
    for (frame = 0U; frame_limit == 0U || frame < frame_limit; ++frame) {
        ...
        /*
         * Front-end focus-group cycling.
         *
         * Hardware run 29 fixed every screen whose MOGA highlight can be
         * moved by the D-pad, but `car_select_new` stays broken. Its trace
         * shows why: once the rollout settles, the layout parks
         * `nav_moga_highlight` on the class menu and A then emits
         * `CLASS_FILTER_SUPER`, which `/menus/garage/garage_select_car`
         * has no output for. No D-pad direction moves that highlight to
         * `btn_car_action_large`, so the Buy/Continue action is only
         * reachable inside the brief pre-settle window.
         *
         * `nativeOnMotionEvent` disassembly supplies the missing input.
         * A full-scale MOGA AXIS_Z (axis 11) is converted to pseudo key
         * codes 190/191 and dispatched as a complete press/release pair,
         * and `MogaKeyCode` turns those into menu actions 1054/1055.
         * That is a real MOGA Pro's right stick, the one front-end input
         * this mapper has never produced. The same table maps 190/191 to
         * no action at all while the guest is in its racing input mode,
         * so this channel cannot affect gameplay.
         *
         * The R36S has no right stick. Its L2/R2 report as SDL axes 4/5
         * and the title ignores the MOGA trigger axes 17/18 completely,
         * so they are free. Each press emits one discrete tap.
         */
        {
            const int focus_threshold = 8192;
            int focus_left = (int)axes[4] > focus_threshold;
            int focus_right = (int)axes[5] > focus_threshold;
            int focus_pulse = 0;

            if (cursor_visible == 0) {
                if (focus_left != 0 && previous_focus_left == 0)
                    focus_pulse = -1;
                else if (focus_right != 0 && previous_focus_right == 0)
                    focus_pulse = 1;
            }
            previous_focus_left = focus_left;
            previous_focus_right = focus_right;
            if (focus_pulse != 0) {
                short hat[6] = { 0, 0, 0, 0, 0, 0 };
                struct fake_object *pulse_event;
                size_t step;

                for (step = 0U; step < 2U; ++step) {
                    hat[2] = step == 0U ?
                        (short)(focus_pulse < 0 ? -32767 : 32767) : (short)0;
                    pulse_event = new_moga_motion(hat);
                    moga_motion(&jni_handle, &moga_listener, pulse_event);
                    free(pulse_event->bytes);
                    free(pulse_event);
                }
                (void)printf("G6-MOGA focus-cycle frame=%u direction=%s "
                             "axis=Z\n", frame,
                             focus_pulse < 0 ? "left" : "right");
            }
        }
